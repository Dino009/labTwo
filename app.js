...
let shops = [new Shop('Пятерочка', 'Russia', []), new Shop('KeBab Shef', 'German', []),
...
app.post('/addGoods', (req, res) => {
    ...
    if (check.length === 0) {
        ...
        res.render('goods', {info: 'Товар успішно додано'})
    } else {
        res.render('goods', {info: 'Перевірте код! Товар з таким кодом уже існує'})
    }
});
...
app.post('/editStorage', (req, res) => {
    ...
    res.render('storage', {data: storage, dataShop: shops, edit: 'Склад проредаговано'})
});
